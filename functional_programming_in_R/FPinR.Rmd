...
# pandoc FPinR.md --slide-level 2 -t beamer -V theme:Berlin -o FPinR.pdf -i
require(reshape2)
require(ggplot2)
data(iris)
#  pandoc FPinR.md --slide-level 2 -t beamer -V theme:warsaw -o FPinR_presentation.pdf -i
```
...
```{r, tidy=FALSE, prompt=TRUE, size = "footnotesize"}
1 + 2
```
...
```{r, tidy = FALSE, size = "footnotesize", prompt=TRUE}
# FP style vectorised operation
a <- 1:200000
x <- a[a %% 2 == 0]
```

```{r, echo = FALSE, size = "footnotesize", prompt=TRUE}
system.time({
    a <- 1:200000
    x <- a[a %% 2 == 0]
})
...
```{r, tidy = FALSE, size = "footnotesize", prompt=TRUE, eval = FALSE}
# e.g.
paste()
colMeans()
rowSums()
